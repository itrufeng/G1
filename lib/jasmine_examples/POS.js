...
POS.prototype.print = function(jsonData) {
  ...
  return this.data.reduce((prev, curr) => {
    ...
    if (this.searchPromotions(curr)) {
      dealName = this.transCodeToInfo(curr).name + ' ' + parseFloat(this.transCodeToInfo(curr).price).toFixed(2) + '元 ' + '[X]';
    } else {
      dealName = this.transCodeToInfo(curr).name + ' ' + parseFloat(this.transCodeToInfo(curr).price).toFixed(2) + '元';
    }
